...
if __name__ == '__main__':
   ...
   #     sys.exit(0)
    ...
    cascade_path = (r"C:\Users\vegetableclean\Anaconda3\envs\tensorflow\Lib\site-packages\cv2\data\haarcascade_frontalface_alt2.xml")   
        ...
        #循环检测识别人脸
    while True:
        frame = cv2.imread(r'D:\ex_111\di.jpg')   #读取一帧视频
            ...
            #浮点并归一化
                ...
                if predict == 0:                                                        
                    ...
                    cv2.rectangle(frame, (x - 10, y - 10), (x + w + 10, y + h + 10), (0,0,255), thickness = 2)
                        ...
                        #文字提示是谁
                    cv2.putText(frame,'woman [%s]'%pred, 
                                (x + 30, y + 30),                      #坐标
                                cv2.FONT_HERSHEY_SIMPLEX,              #字体
                                1,                                     #字号
                                (0,0,255),                           #颜色
                                2)
                    #cv2.putText(frame,'[%s]'%pred[pred>0.5], 
                    #            (x - 20, y + 70),                      #坐标
                    #            cv2.FONT_HERSHEY_SIMPLEX,              #字体
                    #            0.5,                                     #字号
                    #            (255,0,0),                           #颜色
                    #            2) 		
                    cv2.imwrite(r'D:\ex_111\di_hey.jpg', frame)								
                    #plt.imshow(frame) #原始照片
                    #plt.show()
